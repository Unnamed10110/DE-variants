...
public class DE_arithmeticRC {
    ...
    public static void main(String args[]) throws IOException {
        ...
        float combinationCoeff = (float) 0.5;
        ...
        //System.in.read();
                    ...
                    while (cpTime & cpConvergencia) {
                        ...
                        for (int j = 0; j < poblacion.size(); j++) {
                            ...
                            //paso 5: aplicar mutacion Y recombinacion
                            Random rand0 = new Random();
                            float ki = rand0.nextFloat();
                            float mutant[] = new float[x1.size()];
                            for (int k = 0; k < x1.size(); k++) {
                                mutant[k] = poblacion.get(j).get(k).getID() + ki * (x1.get(k).getID() - poblacion.get(j).get(k).getID()) + (ki * M) * (x2.get(k).getID() - x3.get(k).getID());
                            }

                            //paso 6: recombinacion
                            //el vector mutante reemplaza al trial
